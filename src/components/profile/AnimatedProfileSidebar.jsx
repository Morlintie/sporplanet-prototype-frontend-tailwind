...
function AnimatedProfileSidebar() {
  const { isSidebarOpen, closeSidebar, activeSection, setSection } = useProfileSidebar();
  const { favorites } = useFavorites();
  const navigate = useNavigate();

  const menuItems = [
    { id: "profile", label: "Profilim", icon: "user", path: "/profile" },
    { id: "friends", label: "Arkadaşlarım", icon: "friends", path: "/profile?section=friends", badge: "12" },
    { id: "favorite-pitches", label: "Favorilerim", icon: "heart", path: "/profile?section=favorite-pitches", badge: favorites.length.toString() },
    { id: "listings", label: "İlanlarım", icon: "list", path: "/profile?section=listings" },
    { id: "reservations", label: "Rezervasyonlarım", icon: "calendar", path: "/profile?section=reservations" },
    { id: "tournaments", label: "Turnuvalarım", icon: "trophy", path: "/profile?section=tournaments" },
    { id: "messages", label: "Mesajlar", icon: "message", path: "/profile?section=messages" },
    { id: "comments", label: "Yorumlarım", icon: "comment", path: "/profile?section=comments" },
    { id: "invitations", label: "Davetlerim", icon: "mail", path: "/profile?section=invitations" }
  ];

  const settingsItem = { id: "settings", label: "Ayarlar", icon: "settings", path: "/profile?section=settings" };
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-16 right-0 h-[calc(100vh-4rem)] w-80 bg-white shadow-2xl z-40 transform transition-transform duration-300 ease-in-out flex flex-col ${
      ...
      >
        ...
        <nav className="px-4 py-2 space-y-2 overflow-y-auto flex-1">
          {menuItems.map((item) => (
            <button
              key={item.id}
          ...
          <div className="pt-4 mt-4 border-t border-gray-200">
            <button
              ...
              tabIndex="0"
            >
              <span className={`${activeSection === settingsItem.id ? "text-green-600" : "text-gray-400"}`}>
